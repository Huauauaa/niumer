...
const rowClass =
  "group flex w-full cursor-pointer items-center gap-1 rounded px-2 py-1.5 text-left text-[12px] text-[var(--vscode-fg)] hover:bg-[var(--vscode-list-hover)]";
const selectedRow =
  ...
  return (
    ...
    <div className="flex min-h-0 flex-1 flex-col">
                  ...
                  >
                    {editing ? (
                      ...
                      >
                        <input
                          type="text"
                          className="allow-select rounded border border-[#007fd4] bg-[var(--vscode-input-bg)] px-1.5 py-1 text-[12px] text-[var(--vscode-fg)] outline-none"
                          value={draftName}
                        ...
                        <input
                          type="date"
                          className="allow-select rounded border border-[#007fd4] bg-[var(--vscode-input-bg)] px-1.5 py-1 font-mono text-[11px] text-[var(--vscode-fg)] outline-none"
                          value={draftDate}
                        ...
                        <div className="flex gap-1">
                          ...
                          <button
                            type="button"
                            disabled={busy}
                            className="rounded px-2 py-0.5 text-[11px] text-[var(--vscode-fg-muted)] hover:bg-[var(--vscode-menu-hover)] disabled:opacity-50"
                            onClick={cancelEdit}
                          >
                            取消
                          </button>
                      ...
                      <>
                          ...
                          <div className="flex shrink-0 gap-0.5 opacity-0 group-hover:opacity-100">
                            ...
                            >
                              <svg
                                ...
                                viewBox="0 0 24 24"
                                fill="currentColor"
                                className="text-[var(--vscode-fg-muted)]"
                              >
                        ...
                        <div className="font-mono text-[11px] text-[var(--vscode-fg-muted)]">
                          {r.date}
                        </div>
                      </>
                    )}
      ...
      <div className="shrink-0 border-t border-[var(--vscode-border)] px-2 py-2">
        <div className="mb-1.5 text-[10px] font-bold uppercase tracking-wide text-[var(--vscode-fg-muted)]">
          添加
        </div>
        <div className="flex flex-col gap-1.5">
          <input
            type="text"
            className="allow-select w-full rounded border border-[var(--vscode-border)] bg-[var(--vscode-input-bg)] px-2 py-1 text-[12px] text-[var(--vscode-fg)] outline-none focus:border-[#007fd4]"
            placeholder="名称"
            value={newName}
            disabled={busy}
          ...
          <input
            type="date"
            className="allow-select w-full rounded border border-[var(--vscode-border)] bg-[var(--vscode-input-bg)] px-2 py-1 font-mono text-[11px] text-[var(--vscode-fg)] outline-none focus:border-[#007fd4]"
            value={newDate}
            disabled={busy}
    ...
    {pendingDelete ? (
      ...
      >
        <div
          className="w-full max-w-[320px] rounded border border-[var(--vscode-border)] bg-[var(--vscode-dialog-bg)] p-4 shadow-xl"
          role="alertdialog"
          aria-modal="true"
          aria-labelledby="del-title"
          onMouseDown={(e) => e.stopPropagation()}
        >
          <p id="del-title" className="text-[13px] leading-relaxed text-[var(--vscode-fg)]">
